...
test("server-renders the finished Partizan experience", async () => {
  ...
  assert.match(html, /One exact value\./);
  assert.match(html, /Exact pair comparison/);
  assert.match(html, /B and C/);
  assert.match(html, /Two structures are visible\. What survives the change\?/);
  assert.match(html, /Reveal exact relation/);
  assert.match(normalizedHtml, /Form B/);
  assert.match(normalizedHtml, /Form C/);
  assert.match(html, /Difference/);
  assert.match(html, /Technical evidence and JSON/);
  assert.match(html, /Copy evidence JSON/);
  assert.match(html, /Download evidence JSON/);
  assert.match(html, /All three source records/);
  assert.match(html, /og\.png/);
  assert.match(normalizedHtml, /73,728/);
  assert.match(normalizedHtml, /21,697/);
  assert.match(normalizedHtml, /8,111/);
  assert.match(html, /Search proposes positions\. Exact comparison admits them\./);
