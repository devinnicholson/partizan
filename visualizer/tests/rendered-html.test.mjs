...
test("server-renders the finished Partizan experience", async () => {
  ...
  assert.match(html, /Compare the forms\./);
  assert.match(html, /Composer&#x27;s desk/);
  assert.match(normalizedHtml, /v\(A\) = v\(B\) = v\(C\) = 0/);
  assert.match(html, /Select form B/);
  assert.match(html, /Partizan certifies equality\./);
  assert.match(html, /The composer chooses the form\./);
  ...
  assert.match(html, /The study does not assign an aesthetic score\./);
  assert.match(html, /Stiller found the endgame kernel\./);
  assert.match(html, /Elkies composed a study around it\./);
