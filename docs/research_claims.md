...
| C2 | Certified decomposition improves generalization on composed positions. | Blocked by leakage-clean evaluation and modeling. Negative controls and rejected rows show weak, unchecked, or absent strict decomposition must not enter exact training. The Wave 17 fixture shard proves nested certificate transport, leakage accounting, rejected-row exclusion, and baseline wiring. Wave 18 adds small source-aware non-fixture topology and generated-vs-curated holdouts. Wave 19 adds replay verification for the 17 current non-fixture exact rows. Wave 22 expands to 41 replay-verified exact non-fixture composed-board rows with 30 generated rows balanced across generated depth-two topologies, but the Partizan topology/source reports fail leakage gates because generated component/decomposition identities are reused. No learned decomposition model has been trained or compared against controls. | Strict decomposition certificates, verified composed positions, a composition OOD split at scale, and learned decomposition-aware results on a leakage-clean split. | Full-board scalar model, no-decomposition structured model. | Decomposition-aware models do not improve on held-out compositions. | Needs either a no-reuse generated expansion or group-aware split that prevents component/decomposition identity leakage, followed by decomposition-aware baselines. The Wave 22 replay and leakage-audit reports protect against overclaiming but do not prove improvement. | `topology_engineer`, `model_lab` |
